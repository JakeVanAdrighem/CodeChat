...
data Message = Notice String
             ...
             | Command Cmd -- added support for commands
...
runCommand :: [String] -> Server -> Client -> STM ()
runCommand (x:xs) s@Server{..} c@Client{..}
        -- | x == "add" = do broadcast $ s "cmd: add"
        -- | x == "usrlist" = do broadcast $ "cmd: usrlist"
        | x == "quit" = do deleteClient s c
        | otherwise = do broadcast s (MessageFrom clientName ("cmd not found: " ++ x)) 
...
serveLoop server@Server{..}
    ...
    recv_tid <- spawnWorker $ forever $ do
        ...
        case head $ words msg of
            "Cmd:" -> atomically $ runCommand (tail $ words msg) server client
            _ -> atomically $ broadcast server $ MessageFrom clientName msg
