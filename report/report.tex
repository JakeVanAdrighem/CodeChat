...
The graphics library we used was the Gimp ToolKit (GTK).
...
independantly, the decision to have multi-threaded code came easy, and
...
(type inferrence). As a modern language, it offers a robust standard 
...
facilitate communication across goroutines.
...
Simple stress tests performed on the server yielded around 100 
messges/second across a network, and around 500 messages/second on a 
...
\begin{itemize}
	...
	\iteme Number of active clients connected to the server
\end{itemize}
...
used to identify weakpoints in both the client library and the server 
code.
...
one of our original main concerns. Since every time a client recieves a 
...
since everytime the file is updated, the users cursor position is lost. 
